...
void dump_buffer(sycl::queue& queue, void* gpu_addr);
void atomic_write_remote(sycl::queue& queue, int good);
...
int main(int argc, char* argv[]) {
  ...
  atomic_write_remote(queue, (argc > 1));

  MPI_Barrier(MPI_COMM_WORLD);
  std::cout << "Host MPI barrier completed" << std::endl;  

  dump_buffer(queue, ptr);
...
void atomic_write_remote(sycl::queue& queue, int good) {
  ...
  for (int index = 0; index < temp_world; index++) {
    if (index != temp_rank) {
      ...
      std::cout << "Setting " << temp_sync_buffer[index] << " (local mapped) to 1" << std::endl;
    }
  }
...
void exchange_mem(sycl::queue& queue, void* ptr) {
  ...
  void * sync_addr = (void *)((char*)base_addr + send_buf.offset + buffer_base_size);
  dump_buffer(queue, sync_addr);
  ...
  for (uint32_t i = 0; i < world; i++){
    ...
    //        l0_ctx, l0_device, peer->ipc_handle, ZE_IPC_MEMORY_FLAG_BIAS_UNCACHED, &peer_base));
...
void dump_buffer(sycl::queue& queue, void* gpu_addr) {
  ...
  queue.memcpy(host_buffer, gpu_addr, 1024);
  queue.wait();
  std::cout << "Buffer copied from " << gpu_addr << " to host" << std::endl;
  std::cout << "Dump content of " << gpu_addr << ": " << std::endl;
  for (int i = 0; i < world; i++) {
    //if (i != rank) {
    std::cout << (int *)gpu_addr + i * 32 << ": " << host_buffer[i * 32] << std::endl;
    //}
  }
  free(host_buffer);
}
